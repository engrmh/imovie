...
export default function AddNewFilm() {
  ...
  const newFilmGen = () => {
    ...
    fetch("https://moviesapi.ir/api/v1/movies/multi", {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
      body: JSON.stringify(mainFilm),
    })
      .then((result) => {
        Swal.fire({
        ...
        <Row className="p-5 text-white">
          ...
          <Col xs={12} md={6} className="mb-2">
            <Form.Group>
              ...
              <Form.Control
                value={filmCountry}
                onChange={(event) => setFilmCountry(event.target.value)}
                type="text"
                className="newMovieInput bg-opacity-25 bg-white rounded mb-2 border-0 border-bottom border-warning"
                required
              />
          ...
          <Col xs={12} md={6} className="mb-2">
            <Form.Group>
              ...
              <Form.Control
                value={filmYear}
                onChange={(event) => setFilmYear(event.target.value)}
                type="text"
                className="newMovieInput bg-opacity-25 bg-white rounded mb-2 border-0 border-bottom border-warning"
                required
              />
          ...
          <Col
            xs={12}
            className="mb-2 d-flex justify-content-center align-items-center mt-3"
          >
            <Button
